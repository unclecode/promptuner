...
1. In <Inputs> tags, write down the barebones, minimal, nonoverlapping set of text input variable(s) the instructions will make reference to. (These are variable names, not specific instructions.) Some tasks may require only one input variable; rarely will more than four-to-five be required.
...
4. Make sure to wrap variables in XML tags with same name as the variable name. For example, if the variable is {VAR_NAME}. This is helpful for vriables with lengthy content. You should wrap it like:
<VAR_NAME>
{VAR_NAME}
</VAR_NAME>
...
Note: Alwayse demarcated each variable by XML tags, so that the AI knows where the variable starts and ends.
